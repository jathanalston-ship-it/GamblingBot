"""Fixtures for orchestration tests: in-memory DB session and a scan factory."""
...
# Component columns the screener's ScanResult.candidates expects.
...
def make_scan() -> Callable[..., ScanResult]:
    """Build a ScanResult from a list of per-symbol feature dicts."""
    ...
    def _make(rows: list[dict[str, Any]], as_of: str = "2026-01-05") -> ScanResult:
        ...
        for i, row in enumerate(rows, start=1):
            record = {
                ...
                "rank": row.get("rank", i),
                "momentum_score": row.get("momentum_score", 80.0),
                "price": row.get("price", 100.0),
                "volume": row.get("volume", 1_000_000.0),
                "dollar_volume": row.get("dollar_volume", 1.0e8),
                "relative_volume": row.get("relative_volume", 2.0),
                ...
                "sector_rs": row.get("sector_rs", 0.9),
            }
