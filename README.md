...
| **[.github/workflows/release.yml](.github/workflows/release.yml)** | Release CI/CD: **auto-releases on every push to the default branch** (version auto-resolved; `[skip release]` opts out) as well as on a `v*` tag / manual run — quality gate (ruff + mypy + tests + desktop build), build the Windows installer (`build_windows.ps1`), and publish a GitHub Release with `MomentumLab-Setup-*.exe` (+ auto-update metadata). |
...
| **[docs/AUTO_UPDATE.md](docs/AUTO_UPDATE.md)** | **Automatic updates**: releases build on every push, the app checks on launch, and (after one confirmation prompt showing the download size + recommended free disk space) it downloads and installs on its own. Sizing math (`auto-update.ts`), app-side preferences (`update-prefs.ts`), the `unattended` overlay flow, and the manual fallback. |
| **[docs/AUTO_UPDATE_AUDIT.md](docs/AUTO_UPDATE_AUDIT.md)** | Auto-update audit (**resolved** — repo is now public, feed returns 200): why `releases.atom` had 404'd (electron-updater fetches the feed unauthenticated; a private repo's feed 404s), current-vs-expected config, and the detailed, non-suppressed feed diagnostics in the Updates screen. |
