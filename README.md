...
| **[docs/TRADE_LIFECYCLE.md](docs/TRADE_LIFECYCLE.md)** | Trade lifecycle engine: every recommendation becomes a persistent tracked trade, and every scan reevaluates every OPEN trade (conviction delta, momentum/RS/volume trends, ATR expansion, regime/sector/analog changes, thesis strength & stability) into Hold / Scale In / Scale Out / Raise Stop / Lower Stop / Exit — append-only history, never overwritten. |
